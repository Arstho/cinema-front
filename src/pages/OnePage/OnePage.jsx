...
export const OnePage = () => {
  ...
  return (
    <div className={styles.container}>
      <div>
        <h1>
          {movie.name} ({movie.release}) смотреть онлайн
        </h1>
      </div>
      {movie.sub !== "Подписка" ? (
        <iframe
        ...
        <div className={styles.info}>
          <h1>фильм доступен по подписке</h1>
          <Link to='/sub'>
            <h2>желаете приобрести подписку?</h2>
            <p className={styles.sub_link}>перейти на страницу оплаты</p>
          </Link>
        </div>
      )}
      <div className={styles.posterAndInfo}>
        ...
        <div className={styles.info}>
          ...
          <div>
            <div>
              Страна: <a href='#'>{movie.country}</a>{" "}
            </div>
          </div>
          <div>
            <div>
              Жанр: <a href='#'>{movieGenre}</a>{" "}
            </div>
          </div>
          <div>
            <div>
      ...
      <div className={styles.discription}>
        {movie.description?.length < 501 ? (
          movie.description
        ) : fullText?.length >= 501 ? (
          <p className={styles.readBtn}>
            {fullText} <p onClick={hendleFullText}>Скрыть</p>
          </p>
        ) : (
          <p className={styles.readBtn}>
            {fullText}...<p onClick={hendleFullText}>Читать</p>
          </p>
        )}
      </div>
    </div>
